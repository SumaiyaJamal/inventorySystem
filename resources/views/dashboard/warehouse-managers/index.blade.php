...
<head>
	...
	<title>Warehouse Managers | AdminKit Demo</title>
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="d-flex justify-content-between align-items-center mb-3">
						<h1 class="h3 mb-0"><strong>Warehouse Managers</strong> Management</h1>
						<div class="d-flex gap-2">
							<a href="{{ route('dashboard') }}" class="btn btn-secondary">
								<i class="align-middle me-1" data-feather="arrow-left"></i> Back to Dashboard
							</a>
							<a href="{{ route('warehouse-managers.create') }}" class="btn btn-primary">
						...
						<div class="col-12">
							<div class="card">
								<div class="card-header">
									<h5 class="card-title mb-0">Warehouse Managers List</h5>
								</div>
								<div class="card-body">
											...
											<table class="table table-hover my-0">
												<thead>
													<tr>
														...
														<th class="d-none d-md-table-cell">Inventory Type</th>
												...
												<tbody>
													@foreach($warehouseManagers as $warehouseManager)
														<tr>
															...
															<td class="d-none d-md-table-cell">{{ $warehouseManager->location ?? 'N/A' }}</td>
															<td class="d-none d-md-table-cell">
																@if($warehouseManager->inventory_type)
																	<span class="badge bg-{{ $warehouseManager->inventory_type == 'spices' ? 'warning' : 'primary' }}">
																		{{ $warehouseManager->inventory_type == 'spices' ? 'Spices' : 'LED Light & Bulbs' }}
																	</span>
																@else
																	<span class="badge bg-secondary">Not Assigned</span>
																@endif
															</td>
															<td class="d-none d-xl-table-cell">{{ $warehouseManager->created_at->format('M d, Y') }}</td>
															<td>
																<div class="d-flex gap-1">
																	<a href="{{ route('warehouse-managers.edit', $warehouseManager) }}" class="btn btn-sm btn-primary" title="Edit">
																		<i class="align-middle" data-feather="edit-2" style="width: 14px; height: 14px;"></i>
																	</a>
																	<form action="{{ route('warehouse-managers.destroy', $warehouseManager) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this warehouse manager?');">
																		@csrf
																		@method('DELETE')
									...
									@else
										<div class="alert alert-info text-center">
											...
											No warehouse managers found. Please add some warehouse managers to get started.
										</div>
									@endif
								</div>
							</div>
