...
<head>
	...
	<title>Tube Light & Bulbs Inventory | AdminKit Demo</title>
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="d-flex justify-content-between align-items-center mb-3">
						<h1 class="h3 mb-0"><strong>Tube Light & Bulbs</strong> Inventory</h1>
						<div class="d-flex gap-2">
							<a href="{{ route('dashboard') }}" class="btn btn-secondary">
								<i class="align-middle me-1" data-feather="arrow-left"></i> Back to Dashboard
							</a>
							<a href="{{ route('inventory.led-lights.create') }}" class="btn btn-primary">
											...
											<table class="table table-hover my-0">
												<thead>
													<tr>
														<th>Product Name</th>
														<th>Brand</th>
														<th class="d-none d-xl-table-cell">Wattage</th>
														<th class="d-none d-md-table-cell">Type</th>
														<th class="d-none d-md-table-cell">Color Temperature</th>
														<th>Quantity</th>
														...
														<th class="d-none d-xl-table-cell">Warranty (Months)</th>
														<th>Actions</th>
													</tr>
												</thead>
												<tbody>
													@foreach($inventories as $inventory)
														<tr>
															<td>{{ $inventory->product_name ?? 'N/A' }}</td>
															<td>{{ $inventory->brand ?? 'N/A' }}</td>
															<td class="d-none d-xl-table-cell">{{ $inventory->wattage ?? 'N/A' }}</td>
															<td class="d-none d-md-table-cell">{{ $inventory->light_type ?? 'N/A' }}</td>
															<td class="d-none d-md-table-cell">{{ $inventory->color_temperature ?? 'N/A' }}</td>
															...
															<td class="d-none d-xl-table-cell">{{ $inventory->purchase_date ? $inventory->purchase_date->format('M d, Y') : 'N/A' }}</td>
															<td class="d-none d-xl-table-cell">{{ $inventory->warranty_months ?? 'N/A' }}</td>
															<td>
																<div class="d-flex gap-1">
																	<a href="{{ route('inventory.led-lights.edit', $inventory) }}" class="btn btn-sm btn-primary" title="Edit">
																		<i class="align-middle" data-feather="edit-2" style="width: 14px; height: 14px;"></i>
																	</a>
																	<form action="{{ route('inventory.led-lights.destroy', $inventory) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this item?');">
																		@csrf
																		@method('DELETE')
