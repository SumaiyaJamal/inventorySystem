...
<head>
	...
	<title>Inventory Requests | AdminKit Demo</title>
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<div class="d-flex justify-content-between align-items-center mb-3">
						<h1 class="h3 mb-0"><strong>Inventory</strong> Requests</h1>
						<div class="d-flex gap-2">
							<a href="{{ route('dashboard') }}" class="btn btn-secondary">
								<i class="align-middle me-1" data-feather="arrow-left"></i> Back to Dashboard
							</a>
							<a href="{{ route('inventory-requests.create') }}" class="btn btn-primary">
								<i class="align-middle me-1" data-feather="plus"></i> New Request
							</a>
						</div>
					</div>
								...
								<div class="card-body">
											...
											<table class="table table-hover my-0">
												<thead>
													<tr>
														...
														<th class="d-none d-xl-table-cell">Approved/Rejected At</th>
														<th>Actions</th>
													</tr>
												</thead>
												<tbody>
														...
														<tr>
															...
															<td class="d-none d-xl-table-cell">
																@if($request->approved_at)
																	...
																	@if($request->approvedBy)
																		<small class="text-muted d-block">by {{ $request->approvedBy->name }}</small>
																	@endif
																@else
																	-
																@endif
															...
															<td>
																<div class="d-flex gap-1">
																	...
																	@if($request->status == 'pending')
																		<a href="{{ route('inventory-requests.edit', $request) }}" class="btn btn-sm btn-primary" title="Edit">
																			<i class="align-middle" data-feather="edit-2" style="width: 14px; height: 14px;"></i>
																		</a>
																		<form action="{{ route('inventory-requests.destroy', $request) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this request?');">
																			@csrf
																			@method('DELETE')
									...
									@else
										<div class="alert alert-info text-center">
											...
											No inventory requests found. <a href="{{ route('inventory-requests.create') }}">Create your first request</a>.
										</div>
									@endif
								</div>
							</div>
